...
class LecturesRecordsController extends AppController
{
	...
	public function docent_edit($lecture_id, $lecture_date){
		...
		)))
		{
			...
			$this->request->data = $this->LecturesRecord->find('first', $options);
			...
			$rows = [];
			foreach($attendance_infos as $info){
				$rows[$info['LecturesAttendance']['user_id'].'-attendance'] = $info['LecturesAttendance']['status'];
			}
			$this->request->data['LecturesRecord'] += $rows;
